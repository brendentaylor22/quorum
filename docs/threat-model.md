...
| T05       | Body, participant, and room-creation caps; per-source and per-session rate limits; bounded queries               | Load test at the declared limit                                                                 |
...
| T12       | SQLite backup API, integrity and record-count checks, restore refusing an existing volume                        | Encryption, off-host copy, and a restore drill                                                  |
...
| T16       | Scheduled expiry and purge, operator purge command, no cross-room identity                                       | Restore-reapplies-expiry drill                                                                  |
